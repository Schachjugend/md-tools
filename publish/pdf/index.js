...
function pdf(outputPath, options) {
  ...
  var stream = through(function write(data) {
    ...
    tmp.file({ mode: 0644, postfix: '.html' }, function htmlFileCreated(err, tmpHtmlPath, htmlFd) {
      ...
      fs.close(htmlFd, function(err) {
        ...
        fs.readFile(options.runningsPath, 'utf8', function(err, js) {
          ...
          tmp.file({ mode: 0644, postfix: '.js' }, function jsFileCreated(err, tmpRunningsPath, jsFd) {
            if (err) { return self.emit('error', err); }
