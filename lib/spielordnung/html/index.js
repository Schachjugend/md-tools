...
function preProcessHtml (html) {
  ...
  var _ = cheerio.load('<table class="main-table"><thead><tr><th></th><th>Jugendspielordnung</th><th>Ausführungsbestimmungen</th></tr></thead><tbody></tbody></table>', { decodeEntities: false })
  ...
  $(':root > h2').each(function (sectionIx, section) {
    ...
    ol.children('li').each(function (subsectionIx, subsection) {
      ...
      var row = cheerio.load('<tr id="' + id + '"><td><a href="#' + id + '">' + id + '</a></td><td class="sp"></td><td class="ab"></td></tr>', { decodeEntities: false })
...
function scaffold (html, options) {
  ...
  var $ = cheerio.load(scaffold, { decodeEntities: false })
  $('body').append(html)
